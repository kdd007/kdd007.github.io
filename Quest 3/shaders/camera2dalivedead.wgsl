  ...
  */
  ...
  fn geometricProduct(a: MultiVector, b: MultiVector) -> MultiVector {
    ...
    // e01s = e01 , e01e01 = -s   , e01eo0 = ecellSizee0o = -eo1 , e01eo1 = -e0o = eo0
  ...
  fn vertexMain(@location(0) pos: vec2f, @builtin(instance_index) idx: u32) -> VertexOutput {
    let u = idx % cellSize; // we are expecting cellSizexcellSize, so modulo cellSize to get the x index
    let v = idx / cellSize; // divide by cellSize to get the y index
    let uv = vec2f(f32(u), f32(v)) / cellSize; // normalize the coordinates to [0, 1]
    let halfLength = 1.f; // half cell length
    let cellLength = halfLength * 2.f; // full cell length
    let cell = pos / cellSize; // divide the input quad into cellSizexcellSize pieces
    let offset = - halfLength + uv * cellLength + cellLength / cellSize * 0.5; // compute the offset for the instance
    // Apply motor
...
fn fragmentMain(@location(0) cellStatus: f32) -> @location(0) vec4f {
  ...
  else if (cellStatus== 1){//life
       return vec4f(255.f/255, 158.f/255, 145.f/255, 1); // (R, G, B, A)
  }
  else if (cellStatus== 2){//always life
       return vec4f(0.f/255, 255.f/255, 163.f/255, 1); // (R, G, B, A)
  }
  else if (cellStatus== 3){// perma ded
       return vec4f(120.f/255, 120.f/255, 120.f/255, 1); // (R, G, B, A)
  }
  else {
       return vec4f(0.f/255, 0.f/255, 0.f/255, 1); // (R, G, B, A)
  }
