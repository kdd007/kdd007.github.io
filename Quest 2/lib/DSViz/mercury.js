...
export default class mercury extends Standard2DPGAPosedVertexColorObject {
  constructor(device, canvasFormat, pose) {
    ...
    // }
    let _vertices= circleMaker(197, 218, 223, 0.03);
    ...
    this._t = 0;
    this._step = 4.787;
    this._pose0 = [2, 0, 0.5, 0.5, 0.5, 0.5];
  ...
  updateGeometry() {
    ...
    this._pose[0] = this._pose0[0] * 1.5 * Math.cos(angle) - this._pose0[1] * Math.sin(angle); 
    this._pose[1] = this._pose0[0] * Math.sin(angle) + this._pose0[1] *1.5* Math.cos(angle); 
    // Keep same?
